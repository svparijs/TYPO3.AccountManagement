...
- Settings
- roles gebruiken die beschikbaar zijn in PolicyService heeft trouwens de getRoles()
- Routes
- Policy
...
This Security Manager tool is a lightweight authentication wrapper around any given package.
...
For an example heres how the Security.Manager packages secures itself agains unauthorized access.
	...
	resources:
	  methods:
	    ...
	    Security_ManagerAccountMethods: 'method(Security\Manager\Controller\AccountController->(index|new|edit|update|delete)Action())'
	roles:
	  Editor: []
	acls:
	  Editor:
...
Configuring the internal directions for actions.
...
- Settings
- Policy
	- Restrict added to Register Expect RegisterAction
- Translations
	- NL
- AOP
	- Redirection user: Anonymous if Registration is not allowed
...
- Reset password email feature
	- Can only change personal password
...
- Workflow with email verification
- Admin approval
- Give package specific roles when registering